...
def index():
    ...
    if form.validate_on_submit():
        data = form.post.data
        language = TextBlob(data).detect_language() if len(data) >= 3 else ''
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    posts = _user.posts.order_by(Post.timestamp.desc()).paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.user', username=_user.username, page=posts.next_num) if posts.has_next else None
    prev_url = url_for('main.user', username=_user.username, page=posts.prev_num) if posts.has_prev else None
    return render_template('user.html', user=_user, posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def send_message(recipient):
    ...
    form = MessageForm()
    if form.validate_on_submit():
        msg = Message(author=current_user, recipient=_user, body=form.message.data)
        db.session.add(msg)
